...
public class Gyazo extends LinkMedia{
    ...
    protected String expandMediaURL(String browseURL) {
        ImageMatch matcher = new ImageMatch("http://(?:www\\.)?gyazo\\.com/(\\w+)(?:\\.png)?", "http://gyazo.com/%1.png");
        return matcher.getFullPageUrl(browseURL);
    }

    @Override
    protected String expandThumbURL(String browseURL) {
        ImageMatch matcher = new ImageMatch("http://(?:www\\.)?gyazo\\.com/(\\w+)(?:\\.png)?", "http://gyazo.com/thumb/%1.png");
        return matcher.getFullPageUrl(browseURL);
    }
